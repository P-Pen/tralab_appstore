.class public Lcom/arialyy/aria/core/queue/DGroupTaskQueue;
.super Lcom/arialyy/aria/core/queue/AbsTaskQueue;
.source "DGroupTaskQueue.java"


# annotations
.annotation system Ldalvik/annotation/Signature;
    value = {
        "Lcom/arialyy/aria/core/queue/AbsTaskQueue<",
        "Lcom/arialyy/aria/core/task/DownloadGroupTask;",
        "Lcom/arialyy/aria/core/download/DGTaskWrapper;",
        ">;"
    }
.end annotation


# static fields
.field private static volatile INSTANCE:Lcom/arialyy/aria/core/queue/DGroupTaskQueue;


# instance fields
.field private final TAG:Ljava/lang/String;


# direct methods
.method static constructor <clinit>()V
    .locals 0

    return-void
.end method

.method private constructor <init>()V
    .locals 1

    .line 48
    invoke-direct {p0}, Lcom/arialyy/aria/core/queue/AbsTaskQueue;-><init>()V

    .line 36
    invoke-static {p0}, Lcom/arialyy/aria/util/CommonUtil;->getClassName(Ljava/lang/Object;)Ljava/lang/String;

    move-result-object v0

    iput-object v0, p0, Lcom/arialyy/aria/core/queue/DGroupTaskQueue;->TAG:Ljava/lang/String;

    return-void
.end method

.method public static getInstance()Lcom/arialyy/aria/core/queue/DGroupTaskQueue;
    .locals 3

    .line 39
    sget-object v0, Lcom/arialyy/aria/core/queue/DGroupTaskQueue;->INSTANCE:Lcom/arialyy/aria/core/queue/DGroupTaskQueue;

    if-nez v0, :cond_0

    .line 40
    const-class v0, Lcom/arialyy/aria/core/queue/DGroupTaskQueue;

    monitor-enter v0

    .line 41
    :try_start_0
    new-instance v1, Lcom/arialyy/aria/core/queue/DGroupTaskQueue;

    invoke-direct {v1}, Lcom/arialyy/aria/core/queue/DGroupTaskQueue;-><init>()V

    sput-object v1, Lcom/arialyy/aria/core/queue/DGroupTaskQueue;->INSTANCE:Lcom/arialyy/aria/core/queue/DGroupTaskQueue;

    .line 42
    invoke-static {}, Lcom/arialyy/aria/core/event/EventMsgUtil;->getDefault()Lcom/arialyy/aria/core/event/EventMsgUtil;

    move-result-object v1

    sget-object v2, Lcom/arialyy/aria/core/queue/DGroupTaskQueue;->INSTANCE:Lcom/arialyy/aria/core/queue/DGroupTaskQueue;

    invoke-virtual {v1, v2}, Lcom/arialyy/aria/core/event/EventMsgUtil;->register(Ljava/lang/Object;)V

    .line 43
    monitor-exit v0

    goto :goto_0

    :catchall_0
    move-exception v1

    monitor-exit v0
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    throw v1

    .line 45
    :cond_0
    :goto_0
    sget-object v0, Lcom/arialyy/aria/core/queue/DGroupTaskQueue;->INSTANCE:Lcom/arialyy/aria/core/queue/DGroupTaskQueue;

    return-object v0
.end method


# virtual methods
.method public bridge synthetic createTask(Lcom/arialyy/aria/core/wrapper/AbsTaskWrapper;)Lcom/arialyy/aria/core/task/AbsTask;
    .locals 0

    .line 32
    check-cast p1, Lcom/arialyy/aria/core/download/DGTaskWrapper;

    invoke-virtual {p0, p1}, Lcom/arialyy/aria/core/queue/DGroupTaskQueue;->createTask(Lcom/arialyy/aria/core/download/DGTaskWrapper;)Lcom/arialyy/aria/core/task/DownloadGroupTask;

    move-result-object p1

    return-object p1
.end method

.method public createTask(Lcom/arialyy/aria/core/download/DGTaskWrapper;)Lcom/arialyy/aria/core/task/DownloadGroupTask;
    .locals 2

    .line 65
    invoke-super {p0, p1}, Lcom/arialyy/aria/core/queue/AbsTaskQueue;->createTask(Lcom/arialyy/aria/core/wrapper/AbsTaskWrapper;)Lcom/arialyy/aria/core/task/AbsTask;

    .line 67
    iget-object v0, p0, Lcom/arialyy/aria/core/queue/DGroupTaskQueue;->mCachePool:Lcom/arialyy/aria/core/queue/pool/BaseCachePool;

    invoke-virtual {p1}, Lcom/arialyy/aria/core/download/DGTaskWrapper;->getKey()Ljava/lang/String;

    move-result-object v1

    invoke-virtual {v0, v1}, Lcom/arialyy/aria/core/queue/pool/BaseCachePool;->taskExits(Ljava/lang/String;)Z

    move-result v0

    if-nez v0, :cond_0

    iget-object v0, p0, Lcom/arialyy/aria/core/queue/DGroupTaskQueue;->mExecutePool:Lcom/arialyy/aria/core/queue/pool/BaseExecutePool;

    invoke-virtual {p1}, Lcom/arialyy/aria/core/download/DGTaskWrapper;->getKey()Ljava/lang/String;

    move-result-object v1

    invoke-virtual {v0, v1}, Lcom/arialyy/aria/core/queue/pool/BaseExecutePool;->taskExits(Ljava/lang/String;)Z

    move-result v0

    if-nez v0, :cond_0

    .line 68
    invoke-static {}, Lcom/arialyy/aria/core/queue/TaskFactory;->getInstance()Lcom/arialyy/aria/core/queue/TaskFactory;

    move-result-object v0

    .line 69
    invoke-static {}, Lcom/arialyy/aria/core/scheduler/TaskSchedulers;->getInstance()Lcom/arialyy/aria/core/scheduler/TaskSchedulers;

    move-result-object v1

    invoke-virtual {v0, p1, v1}, Lcom/arialyy/aria/core/queue/TaskFactory;->createTask(Lcom/arialyy/aria/core/wrapper/AbsTaskWrapper;Lcom/arialyy/aria/core/listener/ISchedulers;)Lcom/arialyy/aria/core/task/ITask;

    move-result-object p1

    check-cast p1, Lcom/arialyy/aria/core/task/DownloadGroupTask;

    .line 70
    invoke-virtual {p0, p1}, Lcom/arialyy/aria/core/queue/DGroupTaskQueue;->addTask(Lcom/arialyy/aria/core/task/AbsTask;)V

    goto :goto_0

    .line 72
    :cond_0
    iget-object p1, p0, Lcom/arialyy/aria/core/queue/DGroupTaskQueue;->TAG:Ljava/lang/String;

    const-string v0, "\u4efb\u52a1\u5df2\u5b58\u5728"

    invoke-static {p1, v0}, Lcom/arialyy/aria/util/ALog;->w(Ljava/lang/String;Ljava/lang/String;)I

    const/4 p1, 0x0

    :goto_0
    return-object p1
.end method

.method public bridge synthetic createTask(Lcom/arialyy/aria/core/wrapper/AbsTaskWrapper;)Lcom/arialyy/aria/core/task/ITask;
    .locals 0

    .line 32
    check-cast p1, Lcom/arialyy/aria/core/download/DGTaskWrapper;

    invoke-virtual {p0, p1}, Lcom/arialyy/aria/core/queue/DGroupTaskQueue;->createTask(Lcom/arialyy/aria/core/download/DGTaskWrapper;)Lcom/arialyy/aria/core/task/DownloadGroupTask;

    move-result-object p1

    return-object p1
.end method

.method public getMaxTaskNum()I
    .locals 1

    .line 61
    invoke-static {}, Lcom/arialyy/aria/core/AriaConfig;->getInstance()Lcom/arialyy/aria/core/AriaConfig;

    move-result-object v0

    invoke-virtual {v0}, Lcom/arialyy/aria/core/AriaConfig;->getDGConfig()Lcom/arialyy/aria/core/config/DGroupConfig;

    move-result-object v0

    invoke-virtual {v0}, Lcom/arialyy/aria/core/config/DGroupConfig;->getMaxTaskNum()I

    move-result v0

    return v0
.end method

.method public getOldMaxNum()I
    .locals 1

    .line 78
    invoke-static {}, Lcom/arialyy/aria/core/AriaConfig;->getInstance()Lcom/arialyy/aria/core/AriaConfig;

    move-result-object v0

    invoke-virtual {v0}, Lcom/arialyy/aria/core/AriaConfig;->getDGConfig()Lcom/arialyy/aria/core/config/DGroupConfig;

    move-result-object v0

    iget v0, v0, Lcom/arialyy/aria/core/config/DGroupConfig;->oldMaxTaskNum:I

    return v0
.end method

.method getQueueType()I
    .locals 1

    const/4 v0, 0x2

    return v0
.end method

.method public maxTaskNum(Lcom/arialyy/aria/core/event/DGMaxNumEvent;)V
    .locals 0
    .annotation runtime Lcom/arialyy/aria/core/event/Event;
    .end annotation

    .line 57
    iget p1, p1, Lcom/arialyy/aria/core/event/DGMaxNumEvent;->maxNum:I

    invoke-virtual {p0, p1}, Lcom/arialyy/aria/core/queue/DGroupTaskQueue;->setMaxTaskNum(I)V

    return-void
.end method
