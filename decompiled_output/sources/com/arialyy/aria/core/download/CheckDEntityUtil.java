...
public class CheckDEntityUtil implements ICheckEntityUtil {
    ...
    private boolean checkFilePath() {
        ...
        if (TextUtils.isEmpty(tempFilePath)) {
            ...
            return false;
        } else if (!FileUtil.canWrite(new File(tempFilePath).getParent())) {
    ...
    private boolean checkUrl() {
        ...
        if (TextUtils.isEmpty(url)) {
            ...
            String str = this.TAG;
            ALog.e(str, "下载失败，url【" + url + "】错误");
            return false;
        } else if (url.indexOf("://") == -1) {
            String str2 = this.TAG;
            ALog.e(str2, "下载失败，url【" + url + "】不合法");
            return false;
        } else if (TextUtils.isEmpty(this.mWrapper.getTempUrl())) {
            return true;
        } else {
